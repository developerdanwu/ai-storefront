...
export function WrappedCard({
  ...
  return (
    <motion.div
      ...
      className={cn(
        "h-full w-full z-0 relative inset-0 flex flex-col items-center justify-center p-8",
        "overflow-hidden",
        className
      )}
    >
      {children}
      <div
        className={cn("absolute w-full h-full z-0", {
          "bg-noise-xl": noise === "xl",
          "bg-noise-md": noise === "md",
          "bg-noise-sm": noise === "sm",
        })}
      />
      {/* Grid lines overlay - layered for varied spacing */}
